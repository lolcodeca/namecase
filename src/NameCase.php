 ...
 *
 * @author    Lola Slade, lola.slade@gmail.com
 ...
 * @link      https://github.com/lolcodeca/namecase
 */
namespace lolcode;
...
class NameCase {
  ...
  public function name_case($str) {
    ...
    for($i = 0; $i < strlen($str); $i++) {
      ...
      if(preg_match('/[\s]+/', $str[$i]) > 0 || $str[$i] == '-' ||
        $str[$i] == '.' || $str[$i] == ',') {
        $processed_chunks[] = $this->process_name_case_chunk($buffer . $str[$i]);
        $buffer = '';
      } else {
        $buffer .= $str[$i];
      }
    ...
    $processed_chunks[] = $this->process_name_case_chunk($buffer);
   ...
   * @see name_case
   */
  function process_name_case_chunk($str) {
    // Surname prefixes
